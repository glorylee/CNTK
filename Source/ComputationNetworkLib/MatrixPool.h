...
struct MemRequestInfo
{
    ...
    bool workspaceFlag;                         // workspace memory or not, by workspace we indicate whether a memory space will be released very shortly after allocation 
    ...
    MemRequestInfo(DEVICEID_TYPE deviceId, shared_ptr<Matrix<ElemType>>*pMatrixPtr, size_t matrixSize, bool mbScale, bool workspaceFlag, int allocStep)
        :deviceId(deviceId), pMatrixPtr(pMatrixPtr), matrixSize(matrixSize), mbScale(mbScale), workspaceFlag(workspaceFlag), allocStep(allocStep), releaseStep(INT_MAX), memoryId(-1)
    {
    }
...
public:
    ...
    // workspaceFlag is a flag indicating a memory is temporary and will be released very shortly. In the current implementation, all workspace
    ...
    template <class ElemType>
    void RequestAllocate(DEVICEID_TYPE deviceId, shared_ptr<Matrix<ElemType>>*pMatrixPtr, size_t matrixSize, bool mbScale, bool workspaceFlag)
    {
        vector<MemRequestInfo<ElemType>>& memInfoVec = GetMemRequestInfoVec<ElemType>(); 
        MemRequestInfo<ElemType> memInfo(deviceId, pMatrixPtr, matrixSize, mbScale, workspaceFlag, m_stepCounter); 
...
// TODO: Make this a runtime option.
    ...
    void OptimizedMemoryAllocationFunc()
    {
        ...
        std::vector<bool> wsFlagVec = {true, false};
        for (auto& devId : m_deviceIDSet)
        {
            for (auto wsFlag : wsFlagVec)   // we allocate the workspace memory pointers first, and they are not shared with the non-workspace memory requests
            {
                ...
                for (auto& memInfo : memInfoVec)
                {
                    ...
                    if (memInfo.deviceId != devId || memInfo.workspaceFlag != wsFlag || !memInfo.mbScale)
                        continue;
                ...
                for (int i = 0; i < memoryCounter; i++)
                {
                    ...
                    for (auto& memInfo : memInfoVec)
                    {
                        if (memInfo.deviceId == devId && memInfo.workspaceFlag == wsFlag && memInfo.memoryId == i)
