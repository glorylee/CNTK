...
protected:
    ...
    void RequestMatrixFromPool(shared_ptr<Matrix<ElemType>>& matrixPtr, MatrixPool& matrixPool, size_t matrixSize=0, bool mbScale=false, bool workspaceFlag=false)
    {
        if (matrixPtr == nullptr)
        {
            matrixPool.RequestAllocate<ElemType>(m_deviceId, &matrixPtr, matrixSize, mbScale, workspaceFlag);
        }
    }
